...
#pragma omp section
      {	  
	cudaSetDevice(0);
	checkCudaError(cudaMemcpyAsync(h_A, d_A, bytes, cudaMemcpyDeviceToHost, halo_stream[0]), "cudaMemcpy d_A");
	cudaStreamSynchronize(halo_stream[0]);
      }
#pragma omp section
      {			  
	cudaSetDevice(1);
	checkCudaError(cudaMemcpyAsync(h_B, d_B, bytes, cudaMemcpyDeviceToHost, halo_stream[1]), "cudaMemcpy d_B");
	cudaStreamSynchronize(halo_stream[1]);
      }
#pragma omp section
      {	  
	cudaSetDevice(2);
	checkCudaError(cudaMemcpyAsync(h_C, d_C, bytes, cudaMemcpyDeviceToHost, halo_stream[2]), "cudaMemcpy d_C");
	cudaStreamSynchronize(halo_stream[2]);
      }
#pragma omp section
      {			  
	cudaSetDevice(3);
	checkCudaError(cudaMemcpyAsync(h_D, d_D, bytes, cudaMemcpyDeviceToHost, halo_stream[3]), "cudaMemcpy d_D");
