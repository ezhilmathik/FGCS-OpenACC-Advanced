...
#pragma omp section
	{
	  cudaSetDevice(1);
	  checkCudaError(cudaMemcpyPeerAsync(d_B1, 0, d_B2, 1, bytes, halo_stream[1]), "cudaMemcpy d_A");
	  cudaStreamSynchronize(halo_stream[1]);
	}
#pragma omp section
	{
	  cudaSetDevice(2);
	  checkCudaError(cudaMemcpyPeerAsync(d_C1, 0, d_C3, 2, bytes, halo_stream[2]), "cudaMemcpy d_A");
	  cudaStreamSynchronize(halo_stream[2]);
	}
#pragma omp section
	{
	  cudaSetDevice(3);
	  checkCudaError(cudaMemcpyPeerAsync(d_D1, 0, d_D4, 3, bytes, halo_stream[3]), "cudaMemcpy d_A");
