...
#pragma omp section
	{
	  ...
	  cudaSetDevice(0);
	  checkCudaError(cudaMemcpyPeerAsync(d_C, 1, d_A, 0, bytes, halo_stream[0]), "cudaMemcpy d_A");
	  cudaStreamSynchronize(halo_stream[0]);
	}
#pragma omp section
	{
	  cudaSetDevice(1);
	  checkCudaError(cudaMemcpyPeerAsync(d_D, 0, d_B, 1, bytes, halo_stream[1]), "cudaMemcpy d_A");
