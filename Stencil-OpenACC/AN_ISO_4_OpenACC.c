...
void x_copy_from_4(int dk, double *a_4_1, double *c_T4, 
		   int i, int block, int width,int top, int ID)
{ 
  int k;// = blockIdx.x * blockDim.x + threadIdx.x;
  //if( k >= 0 && k < block-2)
       acc_set_device_num(ID, acc_device_nvidia);
		 ...
		 int i, int block, int width, int bot, int ID)
{
  ...
  //if( k >= 0 && k < block-2)
       acc_set_device_num(ID, acc_device_nvidia);
  #pragma acc parallel deviceptr(a_1_2, a_3_1, c_T2) //async(stream) 
#pragma acc loop private (k)
  for(k = 0; k < block-2; ++k)
    {
		 ...
		 int i, int block, int width, int bot, int ID)
{
  ...
  //if( k >= 0 && k < block-2)
         acc_set_device_num(ID, acc_device_nvidia);
  #pragma acc parallel deviceptr(a_2_2, a_4_1, c_T3) //async(stream) 
#pragma acc loop private (k)
  for(k = 0; k < block-2; ++k)
    {
...
void z_copy_from_4(int dk, double *a_4_1, double *c_T4, 
		   int k, int block, int width, int top, int ID)
{ 
  int i;// = blockIdx.x * blockDim.x + threadIdx.x;
  //if( i >= 0 && i < block-2)
        acc_set_device_num(ID, acc_device_nvidia);
...
void y_copy_from_4(int dk, double *a_4_1, double *c_T4, 
		   int j, int block, int width, int top, int ID)
{  
  int k;// = blockIdx.x * blockDim.x + threadIdx.x;
  //if( k >= 0 && k < block-2)
      acc_set_device_num(ID, acc_device_nvidia);
...
void from_k_to_y_direction_T1(double *T1,double *T1_1, double *T1_2, 
			      ...
			      int block, int width, int breadth, int ID)
{
  ...
  */
  ...
  for(sweep = 0; sweep < tot; sweep++)
      ...
      for(i = 1; i < _nx ; i++)
	{
	  ...
	  function_x_down(c_T1, i, 1, 0, 0, DX, dz, dy, 
						DYZP, DXZP, DXYP, F, _ayy, _azz, _axx, width, 0);
	  x_copy_from_1(1, a_1_2, c_T1, i-1, block, width, bot, 0);

	  
	  function_x_down(c_T2, i, 1, 0, 0, DX, dz, dy, 
						DYZP, DXZP, DXYP, F, _ayy, _azz, _axx, width, 1); 
	  x_copy_from_2(1, a_2_1, a_2_2, c_T2, i-1, block, width, top, bot, 1);	

	  
	  function_x_down(c_T3, i, 1, 0, 0, DX, dz, dy, 
						DYZP, DXZP, DXYP, F, _ayy, _azz, _axx, width, 2);
	  x_copy_from_3(1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot, 2);


	  
	  function_x_down(c_T4, i, 1, 0, 0, DX, dz, dy, 
						DYZP, DXZP, DXYP, F, _ayy, _azz, _axx, width, 3);
	  x_copy_from_4(1, a_4_1, c_T4, i-1, block, width, top, 3);
	  ...
	  acc_memcpy_from_device(b_2_2, a_2_2, buff_size); 
	  
	  
	  acc_memcpy_from_device(b_3_1, a_3_1, buff_size);  
	  acc_memcpy_from_device(b_3_2, a_3_2, buff_size);  
	  
	  
	  acc_memcpy_from_device(b_4_1, a_4_1, buff_size);  
	  	  

	  
          acc_memcpy_to_device(a_1_2, b_2_1, buff_size); 
	  x_copy_to_1(1, a_1_2, c_T1, i-1, block, width, bot+1, 0);

          
	  acc_memcpy_to_device(a_2_1, b_1_2, buff_size);
	  acc_memcpy_to_device(a_2_2, b_3_1, buff_size);    
	  x_copy_to_2(1, a_2_1, a_2_2, c_T2, i-1, block, width, bot+1, 1);
	 
	        
	  acc_memcpy_to_device(a_3_1, b_2_2, buff_size);  
	  acc_memcpy_to_device(a_3_2, b_4_1, buff_size);  
	  x_copy_to_3(1, a_3_1, a_3_2, c_T3, i-1, block, width, bot+1, 2);
	  
	  
	  acc_memcpy_to_device(a_4_1, b_3_2, buff_size);  
	  x_copy_to_4(1, a_4_1, c_T4, i-1, block, width,bot+1, 3);
	}
      direction_1 +=  omp_get_wtime();	    
      ...
      for(i = _nx; i > 1 ; i--)
	{
	  ...
	  function_x_up(c_T1, i, -1, 0 , 0, DX, dz, dy, 
					      DYZP, DXZP, DXYP, F, -_ayy, -_azz, -_axx, width, 0);
	  x_copy_from_1(-1, a_1_2, c_T1, i-1, block, width, bot, 0);

	  
	  function_x_up(c_T2, i, -1, 0 , 0, DX, dz, dy, 
					      DYZP, DXZP, DXYP, F, -_ayy, -_azz, -_axx, width, 1);
	  x_copy_from_2(-1, a_2_1, a_2_2, c_T2, i-1, block, width, top,  bot, 1);	

	  
	  function_x_up(c_T3, i, -1, 0 , 0, DX, dz, dy, 
					      DYZP, DXZP, DXYP, F, -_ayy, -_azz, -_axx, width, 2);
	  x_copy_from_3(-1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot, 2);

	  
	  function_x_up(c_T4, i, -1, 0 , 0, DX, dz, dy, 
					      DYZP, DXZP, DXYP, F, -_ayy, -_azz, -_axx, width, 3);
	  x_copy_from_4(-1, a_4_1, c_T4, i-1, block, width, top, 3);
      ...
      for(i = 1; i < _nx; i++)
	{
	  ...
	  function_z_up(c_T1, i, 0, 0, 1, dz, dy, DX, 
					      DXYP, DYZP, DXZP, F, _axx, _ayy, _azz, width, 0);
	  z_copy_from_1(1, a_1_2, c_T1, i-1, block, width, bot, 0);

	  
	  
	  function_z_up(c_T2, i, 0, 0, 1, dz, dy, DX, 
					      DXYP, DYZP, DXZP, F, _axx, _ayy, _azz, width, 1);
	  z_copy_from_2(1, a_2_1, a_2_2, c_T2, i-1, block, width, top,  bot, 1);	

	  
	  
	  function_z_up(c_T3, i, 0, 0, 1, dz, dy, DX, 
					      DXYP, DYZP, DXZP, F, _axx, _ayy, _azz, width, 2);
	  z_copy_from_3(1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot, 2);

	  
	  
	  function_z_up(c_T4, i, 0, 0, 1, dz, dy, DX, 
					      DXYP, DYZP, DXZP, F, _axx, _ayy, _azz, width, 3);
	  z_copy_from_4(1, a_4_1, c_T4, i-1, block, width, top, 3);
	  ...
	  acc_memcpy_from_device(b_4_1, a_4_1, buff_size);              
          ...
          acc_memcpy_to_device(a_1_2, b_2_1, buff_size); 
	  z_copy_to_1(1, a_1_2, c_T1, i-1, block, width, bot+1, 0);
	
	  
          acc_memcpy_to_device(a_2_1, b_1_2, buff_size);
	  acc_memcpy_to_device(a_2_2, b_3_1, buff_size); 
	  z_copy_to_2(1, a_2_1, a_2_2, c_T2, i-1, block, width, bot+1, 1);
	 
	        
	  acc_memcpy_to_device(a_3_1, b_2_2, buff_size);  
	  acc_memcpy_to_device(a_3_2, b_4_1, buff_size);  
	  z_copy_to_3(1, a_3_1, a_3_2, c_T3, i-1, block, width, bot+1, 2);
	  
	  
	  acc_memcpy_to_device(a_4_1, b_3_2, buff_size);  
	  z_copy_to_4(1, a_4_1, c_T4, i-1, block, width, bot+1, 3);
	}
      direction_3 +=  omp_get_wtime();
	...
	{	  
	  ...
	  function_z_down(c_T1, i, 0, 0, 1, dz, dy, DX, 
						DXYP, DYZP, DXZP, F, -_axx, -_ayy, -_azz, width, 0);
	  z_copy_from_1(-1, a_1_2, c_T1, i-1, block, width, bot, 0);


	  
	  function_z_down(c_T2, i, 0, 0, 1, dz, dy, DX, 
						DXYP, DYZP, DXZP, F, -_axx, -_ayy, -_azz, width, 1);
	  z_copy_from_2(-1, a_2_1, a_2_2,c_T2, i-1, block, width, top,  bot, 1);


	  
	  function_z_down(c_T3, i, 0, 0, 1, dz, dy, DX, 
						DXYP, DYZP, DXZP, F, -_axx, -_ayy, -_azz, width, 2);
	  z_copy_from_3(-1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot, 2);


	  
	  function_z_down(c_T4, i, 0, 0, 1, dz, dy, DX, 
						DXYP, DYZP, DXZP, F, -_axx, -_ayy, -_azz, width, 3);
	  z_copy_from_4(-1, a_4_1, c_T4, i-1, block, width, top, 3);
      ...
      to_k_to_y_direction_T1(c_T1, c_T1_1, c_T1_2, 
						  c_T1_3, c_T1_4, block, width, breadth, 0);
      
      // mutilpe GPU biuffer in to 2 GPU
      
      to_k_to_y_direction_T2(c_T2, c_T2_1, c_T2_2, 
						  c_T2_3, c_T2_4, block, width, breadth, 1);
      
      // mutilpe GPU biuffer in to 3 GPU
      
      to_k_to_y_direction_T3(c_T3, c_T3_1, c_T3_2, 
						  c_T3_3, c_T3_4, block, width, breadth, 2);
     
      // mutilpe GPU biuffer in to 4 GPU
      
      to_k_to_y_direction_T4(c_T4, c_T4_1, c_T4_2, 
						  c_T4_3, c_T4_4, block, width, breadth, 3);
      data_timer_1 +=  omp_get_wtime();
      ...
      for(i = _ny; i > 1; i--)
	{
	  ...
	  function_y_up(c_T1, i, 0, -1, 0, dy, DX, dz, 
					      DXZP, DXYP, DYZP, F, -_azz, -_axx, -_ayy, width, 0);
	  y_copy_from_1(-1, a_1_2, c_T1, i-1, block, width, bot, 0);

	  
	  function_y_up(c_T2, i, 0, -1, 0, dy, DX, dz, 
					      DXZP, DXYP, DYZP, F, -_azz, -_axx, -_ayy, width, 1);
	  y_copy_from_2(-1, a_2_1, a_2_2,c_T2, i-1, block, width, top,  bot, 1);	

	  
	  
	  function_y_up(c_T3, i, 0, -1, 0, dy, DX, dz, 
					      DXZP, DXYP, DYZP, F, -_azz, -_axx, -_ayy, width, 2);
	  y_copy_from_3(-1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot, 2);

	  
	  function_y_up(c_T4, i, 0, -1, 0, dy, DX, dz, 
					      DXZP, DXYP, DYZP, F, -_azz, -_axx, -_ayy, width, 3);
	  y_copy_from_4(-1, a_4_1, c_T4, i-1, block, width, top, 3);
	  ...
	  acc_memcpy_from_device(b_4_1, a_4_1, buff_size);  
          ...
          acc_memcpy_to_device(a_1_2, b_2_1, buff_size); 
	  y_copy_to_1(-1, a_1_2, c_T1, i-1, block, width,bot+1, 0);
	 
	  
	  acc_memcpy_to_device(a_2_1, b_1_2, buff_size);
	  acc_memcpy_to_device(a_2_2, b_3_1, buff_size); 
	  y_copy_to_2(-1, a_2_1, a_2_2, c_T2, i-1, block, width, bot+1, 1);
	 
	        
	  acc_memcpy_to_device(a_3_1, b_2_2, buff_size);  
	  acc_memcpy_to_device(a_3_2, b_4_1, buff_size);  
	  y_copy_to_3(-1, a_3_1, a_3_2, c_T3, i-1, block, width, bot+1, 2);
	 
	  
	  acc_memcpy_to_device(a_4_1, b_3_2, buff_size);  
	  y_copy_to_4(-1, a_4_1, c_T4, i-1, block, width, bot+1, 3);  
	}
      direction_5 +=  omp_get_wtime();
      ...
      for(i = 1; i < _ny; i++)
	{
	  ...
	  function_y_down(c_T1, i, 0, 1, 0, dy, DX, dz, 
						DXZP, DXYP, DYZP, F, _azz, _axx, _ayy, width, 0);
	  y_copy_from_1(1, a_1_2, c_T1, i-1, block, width, bot, 0);

	  
	  function_y_down(c_T2, i, 0, 1, 0, dy, DX, dz, 
						DXZP, DXYP, DYZP, F, _azz, _axx, _ayy, width, 1);
	  y_copy_from_2(1, a_2_1, a_2_2, c_T2, i-1, block, width, top,  bot, 1);	
	  
	  
	  function_y_down(c_T3, i, 0, 1, 0, dy, DX, dz, 
						DXZP, DXYP, DYZP, F, _azz, _axx, _ayy, width, 2);
	  y_copy_from_3(1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot, 2);

	  
	  function_y_down(c_T4, i, 0, 1, 0, dy, DX, dz, 
						DXZP, DXYP, DYZP, F, _azz, _axx, _ayy, width, 3);
	  y_copy_from_4(1, a_4_1, c_T4, i-1, block, width, top, 3);
...
#ifdef PRI
  FILE *init;
  init = fopen("an_iso_4_openacc.txt","w");
  for(i = 0; i < block; i++)
    {
      for(j = 0; j < block; j++)
	{
...
#ifdef PRI
  printf(" Solution is written in the file------------------------ an_iso_4_openacc.txt \n");
#endif
  printf("*******************************************************************************************\n");
  return 0;
}
