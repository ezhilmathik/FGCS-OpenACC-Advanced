...
#pragma acc loop private (j, k, ay, az, ax, tnew, st, xt, yt, txy, xnt, ynt, txm, tym, txnyn)
  for(k = 1; k < HEIGHT-1; ++k)
    {
      j = WIDTH-2;
      tnew = fabs((T[(i+1) + HEIGHT * (j + WIDTH * k)]));
...
#ifdef PRI
  FILE *init;
  init = fopen("stream_an_iso_2_openacc.txt","w");  
  for(i = 0; i < _nz+2; i++)
    {
      for(j = 0; j < _nz+2; j++)
	{
...
#ifdef PRI
  printf(" Solution is written in the file------------------------ stream_an_iso_2_openacc.txt\n");
#endif
  printf("*******************************************************************************************\n");
