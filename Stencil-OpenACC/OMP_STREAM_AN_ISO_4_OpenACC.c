...
#pragma omp barrier
	...
	if (tid == 0)      
	  {
	    ...
	    //	    acc_memcpy_device(c_T1_2, c_T2_1, data_size);
	    //	  acc_wait_async(0,2);		    
	    acc_memcpy_to_device(c_T1_3, d_3_1, data_size);
	    //	    acc_memcpy_device(c_T1_3, c_T3_1, data_size);
	    //	  acc_wait_async(0,3);		    
	    acc_memcpy_to_device(c_T1_4, d_4_1, data_size);
	    //	    acc_memcpy_device(c_T1_4, c_T4_1, data_size);
	...
	if (tid == 1)
	  {
	    ...
	    //	    acc_memcpy_device(c_T2_1, c_T1_2, data_size);
	    //	  acc_wait_async(1,2);		    
	    acc_memcpy_to_device(c_T2_3, d_3_2, data_size);
	    //	    acc_memcpy_device(c_T2_3, c_T3_2, data_size);
	    //	  acc_wait_async(1,2);		    
	    acc_memcpy_to_device(c_T2_4, d_4_2, data_size);
	    //	    acc_memcpy_device(c_T2_4, c_T4_2, data_size);
	...
	if (tid == 2)
	  {      
	    ...
	    //	    acc_memcpy_device(c_T3_1, c_T1_3, data_size);
	    //	  acc_wait_async(2,1);		    
	    acc_memcpy_to_device(c_T3_2, d_2_3, data_size);
	    //	    acc_memcpy_device(c_T3_2, c_T2_3, data_size);
	    //	  acc_wait_async(2,3);		    
	    acc_memcpy_to_device(c_T3_4, d_4_3, data_size);
	    //	    acc_memcpy_device(c_T3_4, c_T4_3, data_size);
	...
	if (tid == 3)
	  {
	    ...
	    //	    acc_memcpy_device(c_T4_1, c_T1_4, data_size);
	    //	  acc_wait_async(3,1);		    
	    acc_memcpy_to_device(c_T4_2, d_2_4, data_size);
	    //	    acc_memcpy_device(c_T4_2, c_T2_4, data_size);
	    //	  acc_wait_async(3,2);		    
	    acc_memcpy_to_device(c_T4_3, d_3_4, data_size);
	    //	    acc_memcpy_device(c_T4_3, c_T3_4, data_size);
...
#ifdef PRI
  FILE *init;
  init = fopen("omp_stream_an_iso_4_openacc.txt","w");
  for(i = 0; i < block; i++)
    {
      for(j = 0; j < block; j++)
	{
...
#ifdef PRI
  printf(" Solution is written in the file------------------------ omp_stream_an_iso_4_openacc.txt \n");
#endif
  printf("*******************************************************************************************\n");
  return 0;
}
