...
{	
  ...
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T) async(stream) 
#pragma acc loop collapse(2) private (j,k, ax, ay, ax, tnew, st, xt, yt, txy, xnt, ynt, txm, tym, txnyn)
  for(j = 1; j < WIDTH-2; ++j)
    {
      for(k = 1; k < HEIGHT-1; ++k)
	{       
...
{  
  ...
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T) async(stream)
#pragma acc loop collapse(2) private (i,k, tnew, st, xt, yt, txy, xnt, ynt, txm, tym, txnyn)
  for(i = 1; i < WIDTH-2; ++i)
    {
      for(k = 1; k < HEIGHT-1; ++k)
        {
  ...
  
{
  ...
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T, a_1_2) async(stream) 
#pragma acc loop private (i,k, tnew, st, xt, yt, txy, xnt, ynt, txm, tym, txnyn)
  for(k = 1; k < HEIGHT-1; ++k)
    {
      i=WIDTH-2;
      tnew = fabs((T[i+ WIDTH *((j-1)+HEIGHT*k)]));
  ...
  
{
  ...
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T) async(stream)
#pragma acc loop collapse(2) private (i,j, tnew, st, xt, yt, txy, xnt, ynt, txm, tym, txnyn)
  for(j = 1; j < WIDTH-2; ++j)
    {
      for(i = 1; i < HEIGHT-1; ++i)
        { 
  ...
  
{
  ...
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T, a_2_1) async(stream) 
#pragma acc loop private (i,j, tnew, st, xt, yt, txy, xnt, ynt, txm, tym, txnyn)
  for(i = 1; i < HEIGHT-1; ++i)
    {
      j = 1;
      tnew = fabs((T[i + HEIGHT * (j + WIDTH * (k-1))]));
...
void from_k_to_y_direction_T1(double* T1, double *T1_1, double *T1_2, 
			      int block, int WIDTH, int length, int ID, int stream)
{
  int i, j, k;
  
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T1, T1_1, T1_2) async(stream)
			      ...
			      int block, int WIDTH, int length, int ID, int stream)
{
  ...
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T2, T2_1, T2_2) async(stream)
#pragma acc loop collapse(3) private(i, j, k)
  for(i = 0; i < WIDTH; ++i){
    for(j = 0; j < WIDTH; ++j){
      for(k = 0; k < block; ++k){
			    ...
			    int block, int WIDTH, int length, int ID, int stream)
{
  ...
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T1, T1_1, T2_1) async(stream)
#pragma acc loop collapse(3) private(i, j, k)
  for(i = 0; i < WIDTH; ++i){
    for(j = 0; j < WIDTH; ++j){
      for(k = 0; k < block; ++k){
			    ...
			    int block, int WIDTH, int length, int ID, int stream)
{
  ...
  acc_set_device_num(ID, acc_device_nvidia);
#pragma acc parallel deviceptr(T2, T1_2, T2_2) async(stream)
#pragma acc loop collapse(3) private(i, j, k)
  for(i = 0; i < WIDTH; ++i){
    for(j = 0; j < WIDTH; ++j){
      for(k = 0; k < block; ++k){
...
int main()
{
  ...
  _DOUBLE_ *restrict d_1_2 = NULL; // GPU partial data transfer
  _DOUBLE_ *restrict d_2_1 = NULL; // GPU partial data transfer
...
#pragma omp barrier
	...
	if (tid == 0)
	  {
	    from_k_to_y_direction_T1(c_T1, c_T1_1, c_T1_2, block, height, width, 0, 0);
	    acc_memcpy_device_async(d_2_1, c_T1_2, data_size, 0);
	  }
	if (tid == 1)
	  {
	    from_k_to_y_direction_T2(c_T2, c_T2_1, c_T2_2, block, height, width, 1, 1);
	    acc_memcpy_device_async(d_1_2, c_T2_1, data_size, 1);
	  }
	acc_wait_all();	    		
#pragma omp barrier	
	if (tid == 0)
	  {
	    to_k_to_y_direction_T1(c_T1, c_T1_1, d_1_2, block, height, width, 0, 0);
	  }
	if (tid == 1)
	  {
	    to_k_to_y_direction_T2(c_T2, d_2_1, c_T2_2, block, height, width, 1, 1);
	  }
	acc_wait_all();	    		
#pragma omp barrier
...
#pragma omp barrier
	...
	if (tid == 0)
	  {	      
	    from_y_to_k_direction_T11(c_T1, c_T1_1, c_T1_2, block, height, width, 0, 0);
	    start_data_timer_2 = omp_get_wtime();
	    acc_memcpy_device_async(d_2_1, c_T1_2, data_size, 0);
	    acc_wait(0);
	...
	if (tid == 1)
	  {
	    from_y_to_k_direction_T21(c_T2, c_T2_1, c_T2_2, block, height, width, 1, 1);
	    acc_memcpy_device_async(d_1_2, c_T2_1, data_size, 1);	 
	  }
	acc_wait_all();	    	
#pragma omp barrier
	if (tid == 0)
	  {
	    ...
	    //	    acc_wait(1);
	    //	    start_data_timer_2 = omp_get_wtime();
	    to_y_to_k_direction_T11(c_T1, c_T1_1, d_1_2, block, height, width, 0, 0);
	    //	    acc_wait_all();	    	
	    //	    end_data_timer_2 = omp_get_wtime();
	    //	    data2 += (end_data_timer_2 - start_data_timer_2);	    	    
	  }
	if (tid == 1)
	  {
	    ...
	    //	    acc_wait(0);
	    //	    acc_wait(1);	    
	    to_y_to_k_direction_T21(c_T2, d_2_1, c_T2_2, block, height, width, 1, 1);
	  }
	acc_wait_all();	    	
#pragma omp barrier
	//	end_data_timer_2 = omp_get_wtime();
...
#ifdef PRI
  FILE *init;
  init = fopen("p2p_omp_stream_an_iso_2_openacc.txt","w");  
  for(i = 0; i < _nz+2; i++)
    {
      for(j = 0; j < _nz+2; j++)
	{
...
#ifdef PRI
  printf(" Solution is written in the file------------------------ p2p_omp_stream_an_iso_2_openacc.txt\n");
#endif
  printf("*******************************************************************************************\n");
