...
int main()
{
  ...
  for(sweep = 0; sweep < tot; sweep++)
      ...
      for(i = _ny; i > 1; i--)
	{
	  ...
	  // synchronize all the compute streams
	  cudaSetDevice(0);
	  cudaStreamSynchronize(compute_stream[0]);	  
	  cudaSetDevice(1);
	  cudaStreamSynchronize(compute_stream[1]);	  
...
#ifdef PRI
  FILE *init;
  init = fopen("stream_an_iso_4_gpu.txt","w");
  for(i = 0; i < block; i++)
    {
      for(j = 0; j < block; j++)
	{
...
#ifdef PRI
  printf(" Solution is written in the file------------------------ stream_an_iso_4_gpu.txt \n");
#endif
  printf("*******************************************************************************************\n");
  return 0;
}
