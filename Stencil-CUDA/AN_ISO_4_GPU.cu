...
__global__
void function_x_down(double *T, int i, int di, int dj, int dk, 
...
__global__
void function_x_up(double *T, int i, int di, int dj, int dk, 
		   double dzz, double dxx, double dyy, 
		   double dxy, double dxz, double dyz, 
		   double F, double ay, double ax, double az, int WIDTH)
{
...
__global__
void function_y_up(double *T, int j, int di, int dj, int dk, 
		   double dzz, double dxx, double dyy, 
		   double dxy, double dxz, double dyz, 
		   double F, double ay, double ax, double az, int WIDTH)
{
...
__global__
void function_y_down(double *T, int j, int di, int dj, int dk, 
		     double dzz, double dxx, double dyy, 
		     double dxy, double dxz, double dyz, 
		     double F, double ay, double ax, double az, int WIDTH)
{
...
__global__
void function_z_up(double *T, int k, int di, int dj, int dk, 
		   double dzz, double dxx, double dyy, 
		   double dxy, double dxz, double dyz, 
		   double F, double ay, double ax, double az, int WIDTH)
...
__global__
void function_z_down(double *T, int k, int di, int dj, int dk, 
		     double dzz, double dxx, double dyy, 
		     double dxy, double dxz, double dyz, 
		     double F, double ay, double ax, double az, int WIDTH)
{
...
__global__
void x_copy_to_2(int dk, double *a_1_2, double *a_3_1,double *c_T2, 
		 int i, int block, int width, int bot)
{
  int k = blockIdx.x * blockDim.x + threadIdx.x;
  if( k >= 0 && k < block-2)
...
__global__
void x_copy_to_3(int dk, double *a_2_2, double *a_4_1, double *c_T3, 
		 int i, int block, int width, int bot)
{
  int k = blockIdx.x * blockDim.x + threadIdx.x;
  if( k >= 0 && k < block-2)
...
__global__
void z_copy_to_2(int dk, double *a_1_2, double *a_3_1, double *c_T2, 
		 int k, int block, int width, int bot)
{ 
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if( i >= 0 && i < block-2)
...
__global__
void z_copy_to_3(int dk, double *a_2_2, double *a_4_1, double *c_T3, 
		 int k, int block, int width, int bot)
{
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if( i >= 0 && i < block-2)
...
__global__
void y_copy_to_2(int dk, double *a_1_2, double *a_3_1,double *c_T2, 
		 int j, int block, int width, int bot)
{
...
__global__
void y_copy_to_3(int dk, double *a_2_2, double *a_4_1, double *c_T3, 
		 int j, int block, int width, int bot)
{
  int k = blockIdx.x * blockDim.x + threadIdx.x;
  if( k >= 0 && k < block-2)
...
int main()
{
  ...
  double DX = ( _xmax - _xmin) / (_nx - 1.0);
  double dy = ( _ymax - _ymin) / (_ny - 1.0);
  double dz = ( _zmax - _zmin) / (_nz - 1.0);
  
  double DXYP = sqrt(DX*DX + dy*dy);
  double DXZP = sqrt(DX*DX + dz*dz);
  double DYZP = sqrt(dy*dy + dz*dz);
  double DXYZP =  sqrt(DX*DX + dy*dy + dz*dz);
  ...
  _DOUBLE_ *b_1_2 = (_DOUBLE_*)malloc(sizeof(_DOUBLE_) * _nx); // CPU buff for 1 boundary transfer
  _DOUBLE_ *b_2_1 = (_DOUBLE_*)malloc(sizeof(_DOUBLE_) * _nx); // CPU buff for 2 boundary transfer
  _DOUBLE_ *b_2_2 = (_DOUBLE_*)malloc(sizeof(_DOUBLE_) * _nx); // CPU buff for 2 boundary transfer 
  _DOUBLE_ *b_3_1 = (_DOUBLE_*)malloc(sizeof(_DOUBLE_) * _nx); // CPU buff for 3 boundary transfer 
  _DOUBLE_ *b_3_2 = (_DOUBLE_*)malloc(sizeof(_DOUBLE_) * _nx); // CPU buff for 3 boundary transfer 
  _DOUBLE_ *b_4_1 = (_DOUBLE_*)malloc(sizeof(_DOUBLE_) * _nx); // CPU buff for 4 boundary transfer 
  ...
  for(sweep = 0; sweep < tot; sweep++)
	...
	{
	  cudaSetDevice(0);
	  function_x_down<<< comp_G, comp_B >>>(c_T1, i, 1, 0, 0, DX, dz, dy, 
						DYZP, DXZP, DXYP, F, _ayy, _azz, _axx, width);
	  x_copy_from_1<<< halo_G, halo_B >>>(1, a_1_2, c_T1, i-1, block, width, bot);

	  cudaSetDevice(1);
	  function_x_down<<< comp_G, comp_B >>>(c_T2, i, 1, 0, 0, DX, dz, dy, 
						DYZP, DXZP, DXYP, F, _ayy, _azz, _axx, width); 
	  x_copy_from_2<<< halo_G, halo_B >>>(1, a_2_1, a_2_2, c_T2, i-1, block, width, top, bot);	

	  cudaSetDevice(2);
	  function_x_down<<< comp_G, comp_B >>>(c_T3, i, 1, 0, 0, DX, dz, dy, 
						DYZP, DXZP, DXYP, F, _ayy, _azz, _axx, width);
	  x_copy_from_3<<< halo_G, halo_B >>>(1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot);


	  cudaSetDevice(3);
	  function_x_down<<< comp_G, comp_B >>>(c_T4, i, 1, 0, 0, DX, dz, dy, 
						DYZP, DXZP, DXYP, F, _ayy, _azz, _axx, width);
	  x_copy_from_4<<< halo_G, halo_B >>>(1, a_4_1, c_T4, i-1, block, width, top);
	  ...
	  cudaMemcpy(b_2_2, a_2_2, buff_size, cudaMemcpyDeviceToHost); 
	  
	  cudaSetDevice(2);
	  cudaMemcpy(b_3_1, a_3_1, buff_size, cudaMemcpyDeviceToHost);  
	  cudaMemcpy(b_3_2, a_3_2, buff_size, cudaMemcpyDeviceToHost);  
	  
	  cudaSetDevice(3);
	  cudaMemcpy(b_4_1, a_4_1, buff_size, cudaMemcpyDeviceToHost);  
	  	  

	  cudaSetDevice(0);
          cudaMemcpy(a_1_2, b_2_1, buff_size, cudaMemcpyHostToDevice); 
	  x_copy_to_1<<< halo_G, halo_B >>>(1, a_1_2, c_T1, i-1, block, width, bot+1);

          cudaSetDevice(1);
	  cudaMemcpy(a_2_1, b_1_2, buff_size, cudaMemcpyHostToDevice);
	  cudaMemcpy(a_2_2, b_3_1, buff_size, cudaMemcpyHostToDevice);    
	  x_copy_to_2<<< halo_G, halo_B >>>(1, a_2_1, a_2_2, c_T2, i-1, block, width, bot+1);
	 
	  cudaSetDevice(2);      
	  cudaMemcpy(a_3_1, b_2_2, buff_size, cudaMemcpyHostToDevice);  
	  cudaMemcpy(a_3_2, b_4_1, buff_size, cudaMemcpyHostToDevice);  
	  x_copy_to_3<<< halo_G, halo_B >>>(1, a_3_1, a_3_2, c_T3, i-1, block, width, bot+1);
	  
	  cudaSetDevice(3);
	  cudaMemcpy(a_4_1, b_3_2, buff_size, cudaMemcpyHostToDevice);  
	  x_copy_to_4<<< halo_G, halo_B >>>(1, a_4_1, c_T4, i-1, block, width,bot+1);
	}
      direction_1 += omp_get_wtime();      
	...
	{
	  cudaSetDevice(0);
	  function_x_up<<< comp_G, comp_B >>>(c_T1, i, -1, 0 , 0, DX, dz, dy, 
					      DYZP, DXZP, DXYP, F, -_ayy, -_azz, -_axx, width);
	  x_copy_from_1<<< halo_G, halo_B >>>(-1, a_1_2, c_T1, i-1, block, width, bot);

	  cudaSetDevice(1);
	  function_x_up<<< comp_G, comp_B >>>(c_T2, i, -1, 0 , 0, DX, dz, dy, 
					      DYZP, DXZP, DXYP, F, -_ayy, -_azz, -_axx, width);
	  x_copy_from_2<<< halo_G, halo_B >>>(-1, a_2_1, a_2_2, c_T2, i-1, block, width, top,  bot);	

	  cudaSetDevice(2);
	  function_x_up<<< comp_G, comp_B >>>(c_T3, i, -1, 0 , 0, DX, dz, dy, 
					      DYZP, DXZP, DXYP, F, -_ayy, -_azz, -_axx, width);
	  x_copy_from_3<<< halo_G, halo_B >>>(-1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot);

	  cudaSetDevice(3);
	  function_x_up<<< comp_G, comp_B >>>(c_T4, i, -1, 0 , 0, DX, dz, dy, 
					      DYZP, DXZP, DXYP, F, -_ayy, -_azz, -_axx, width);
	  x_copy_from_4<<< halo_G, halo_B >>>(-1, a_4_1, c_T4, i-1, block, width, top);
	...
	{
	  cudaSetDevice(0);
	  function_z_up<<< comp_G, comp_B >>>(c_T1, i, 0, 0, 1, dz, dy, DX, 
					      DXYP, DYZP, DXZP, F, _axx, _ayy, _azz, width);
	  z_copy_from_1<<< halo_G, halo_B >>>(1, a_1_2, c_T1, i-1, block, width, bot);

	  
	  cudaSetDevice(1);
	  function_z_up<<< comp_G, comp_B >>>(c_T2, i, 0, 0, 1, dz, dy, DX, 
					      DXYP, DYZP, DXZP, F, _axx, _ayy, _azz, width);
	  z_copy_from_2<<< halo_G, halo_B >>>(1, a_2_1, a_2_2, c_T2, i-1, block, width, top,  bot);	

	  
	  cudaSetDevice(2);
	  function_z_up<<< comp_G, comp_B >>>(c_T3, i, 0, 0, 1, dz, dy, DX, 
					      DXYP, DYZP, DXZP, F, _axx, _ayy, _azz, width);
	  z_copy_from_3<<< halo_G, halo_B >>>(1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot);

	  
	  cudaSetDevice(3);
	  function_z_up<<< comp_G, comp_B >>>(c_T4, i, 0, 0, 1, dz, dy, DX, 
					      DXYP, DYZP, DXZP, F, _axx, _ayy, _azz, width);
	  z_copy_from_4<<< halo_G, halo_B >>>(1, a_4_1, c_T4, i-1, block, width, top);
	  ...
	  cudaSetDevice(0);
          cudaMemcpy(a_1_2, b_2_1, buff_size, cudaMemcpyHostToDevice); 
	  z_copy_to_1<<< halo_G, halo_B >>>(1, a_1_2, c_T1, i-1, block, width, bot+1);
	
	  cudaSetDevice(1);
          cudaMemcpy(a_2_1, b_1_2, buff_size, cudaMemcpyHostToDevice);
	  cudaMemcpy(a_2_2, b_3_1, buff_size, cudaMemcpyHostToDevice); 
	  z_copy_to_2<<< halo_G, halo_B >>>(1, a_2_1, a_2_2, c_T2, i-1, block, width, bot+1);
	 
	  cudaSetDevice(2);      
	  cudaMemcpy(a_3_1, b_2_2, buff_size, cudaMemcpyHostToDevice);  
	  cudaMemcpy(a_3_2, b_4_1, buff_size, cudaMemcpyHostToDevice);  
	  z_copy_to_3<<< halo_G, halo_B >>>(1, a_3_1, a_3_2, c_T3, i-1, block, width, bot+1);
	  
	  cudaSetDevice(3);
	  cudaMemcpy(a_4_1, b_3_2, buff_size, cudaMemcpyHostToDevice);  
	  z_copy_to_4<<< halo_G, halo_B >>>(1, a_4_1, c_T4, i-1, block, width, bot+1);
	}
      direction_3 += omp_get_wtime();
	...
	{	  
	  cudaSetDevice(0);
	  function_z_down<<< comp_G, comp_B >>>(c_T1, i, 0, 0, 1, dz, dy, DX, 
						DXYP, DYZP, DXZP, F, -_axx, -_ayy, -_azz, width);
	  z_copy_from_1<<< halo_G, halo_B >>>(-1, a_1_2, c_T1, i-1, block, width, bot);


	  cudaSetDevice(1);
	  function_z_down<<< comp_G, comp_B >>>(c_T2, i, 0, 0, 1, dz, dy, DX, 
						DXYP, DYZP, DXZP, F, -_axx, -_ayy, -_azz, width);
	  z_copy_from_2<<< halo_G, halo_B >>>(-1, a_2_1, a_2_2,c_T2, i-1, block, width, top,  bot);


	  cudaSetDevice(2);
	  function_z_down<<< comp_G, comp_B >>>(c_T3, i, 0, 0, 1, dz, dy, DX, 
						DXYP, DYZP, DXZP, F, -_axx, -_ayy, -_azz, width);
	  z_copy_from_3<<< halo_G, halo_B >>>(-1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot);


	  cudaSetDevice(3);
	  function_z_down<<< comp_G, comp_B >>>(c_T4, i, 0, 0, 1, dz, dy, DX, 
						DXYP, DYZP, DXZP, F, -_axx, -_ayy, -_azz, width);
	  z_copy_from_4<<< halo_G, halo_B >>>(-1, a_4_1, c_T4, i-1, block, width, top);
	...
	{
	  cudaSetDevice(0);
	  function_y_up<<< comp_G, comp_B >>>(c_T1, i, 0, -1, 0, dy, DX, dz, 
					      DXZP, DXYP, DYZP, F, -_azz, -_axx, -_ayy, width);
	  y_copy_from_1<<< halo_G, halo_B >>>(-1, a_1_2, c_T1, i-1, block, width, bot);

	  cudaSetDevice(1);
	  function_y_up<<< comp_G, comp_B >>>(c_T2, i, 0, -1, 0, dy, DX, dz, 
					      DXZP, DXYP, DYZP, F, -_azz, -_axx, -_ayy, width);
	  y_copy_from_2<<< halo_G, halo_B >>>(-1, a_2_1, a_2_2,c_T2, i-1, block, width, top,  bot);	

	  
	  cudaSetDevice(2);
	  function_y_up<<< comp_G, comp_B >>>(c_T3, i, 0, -1, 0, dy, DX, dz, 
					      DXZP, DXYP, DYZP, F, -_azz, -_axx, -_ayy, width);
	  y_copy_from_3<<< halo_G, halo_B >>>(-1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot);

	  cudaSetDevice(3);
	  function_y_up<<< comp_G, comp_B >>>(c_T4, i, 0, -1, 0, dy, DX, dz, 
					      DXZP, DXYP, DYZP, F, -_azz, -_axx, -_ayy, width);
	  y_copy_from_4<<< halo_G, halo_B >>>(-1, a_4_1, c_T4, i-1, block, width, top);
	  ...
	  cudaSetDevice(0);
          cudaMemcpy(a_1_2, b_2_1, buff_size, cudaMemcpyHostToDevice); 
	  y_copy_to_1<<< halo_G, halo_B >>>(-1, a_1_2, c_T1, i-1, block, width,bot+1);
	 
	  cudaSetDevice(1);
	  cudaMemcpy(a_2_1, b_1_2, buff_size, cudaMemcpyHostToDevice);
	  cudaMemcpy(a_2_2, b_3_1, buff_size, cudaMemcpyHostToDevice); 
	  y_copy_to_2<<< halo_G, halo_B >>>(-1, a_2_1, a_2_2, c_T2, i-1, block, width, bot+1);
	 
	  cudaSetDevice(2);      
	  cudaMemcpy(a_3_1, b_2_2, buff_size, cudaMemcpyHostToDevice);  
	  cudaMemcpy(a_3_2, b_4_1, buff_size, cudaMemcpyHostToDevice);  
	  y_copy_to_3<<< halo_G, halo_B >>>(-1, a_3_1, a_3_2, c_T3, i-1, block, width, bot+1);
	 
	  cudaSetDevice(3);
	  cudaMemcpy(a_4_1, b_3_2, buff_size, cudaMemcpyHostToDevice);  
	  y_copy_to_4<<< halo_G, halo_B >>>(-1, a_4_1, c_T4, i-1, block, width, bot+1);  
	}
      direction_5 +=  omp_get_wtime();
	...
	{
	  cudaSetDevice(0);
	  function_y_down<<< comp_G, comp_B >>>(c_T1, i, 0, 1, 0, dy, DX, dz, 
						DXZP, DXYP, DYZP, F, _azz, _axx, _ayy, width);
	  y_copy_from_1<<< halo_G, halo_B >>>(1, a_1_2, c_T1, i-1, block, width, bot);

	  cudaSetDevice(1);
	  function_y_down<<< comp_G, comp_B >>>(c_T2, i, 0, 1, 0, dy, DX, dz, 
						DXZP, DXYP, DYZP, F, _azz, _axx, _ayy, width);
	  y_copy_from_2<<< halo_G, halo_B >>>(1, a_2_1, a_2_2, c_T2, i-1, block, width, top,  bot);	
	  
	  cudaSetDevice(2);
	  function_y_down<<< comp_G, comp_B >>>(c_T3, i, 0, 1, 0, dy, DX, dz, 
						DXZP, DXYP, DYZP, F, _azz, _axx, _ayy, width);
	  y_copy_from_3<<< halo_G, halo_B >>>(1, a_3_1, a_3_2, c_T3, i-1, block, width, top, bot);

	  cudaSetDevice(3);
	  function_y_down<<< comp_G, comp_B >>>(c_T4, i, 0, 1, 0, dy, DX, dz, 
						DXZP, DXYP, DYZP, F, _azz, _axx, _ayy, width);
	  y_copy_from_4<<< halo_G, halo_B >>>(1, a_4_1, c_T4, i-1, block, width, top);
...
#ifdef PRI
  printf(" Solution is written in the file------------------------ an_iso_4_gpu.txt \n");
#endif
  printf("*******************************************************************************************\n");
  return 0;
}
