		   ...
		   int k, int block, int WIDTH, int bot)
{
  ...
  if ( i >= 0 && i < block-2)
    {
      a_1_2[i] = __ldg(&c_T1[(i+1) + block* (bot + WIDTH * (k+dk+1))]);
    }
}
...
{  
  ...
  if ( i >= 0 && i < block-2)
    {
      a_2_1[i] = __ldg(&c_T2[(i+1) + block * (top + WIDTH * (k+dk+1))]); 
    }
}
...
__global__
void z_copy_to_2(int dk, const double* __restrict a_1_2, double *c_T2, 
		 int k, int block, int WIDTH, int bot)
{ 
  int i = blockIdx.x * blockDim.x + threadIdx.x;
  if ( i >= 0 && i < block-2)
...
// Save top bounday layer from T1
__global__
void x_copy_from_1(int dk, double *a_1_2, const double* __restrict c_T1, 
		 ...
		 int block, int WIDTH, int bot)
{
  ...
  if ( k >= 0 && k < block-2)
    {
      c_T2[0 + WIDTH * ((j+1+dk) + block * (k+1))]  = __ldg(&a_1_2[k]); 
    }
}
...
#ifdef PRI
  FILE *init;
  init = fopen("omp_an_iso_2_stream_gpu.txt","w");  
  for(i = 0; i < _nz+2; i++)
    {
      for(j = 0; j < _nz+2; j++)
	{
...
#ifdef PRI
  printf(" Solution is written in the file------------------------ omp_an_iso_2_stream_gpu.txt\n");
#endif
  printf("*******************************************************************************************\n");
